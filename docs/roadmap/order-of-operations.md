...
**Provenance, because it changes how to read this file.** The build sequence and several
decisions below were derived by reading the predecessor, `flight-path-hud`, for what to
port. That project began as **read-only MAVLink**, so some of what it settled encodes
assumptions this repo does not share — we extend well past reading. Divergence from it is
therefore expected, and where this document diverges the reason is stated inline.

**Where this document and an executable artifact disagree, the artifact wins and this
document is the defect** (ADR-0008 §3). The executable artifacts are
`docs/reference/codec-capability-matrix.md` (enforced by `TestMatrixCoverage`),
`scripts/check-*.sh`, `contracts/mavlink/`, and the `.proto` files. This file states
intent; those state fact.
...
| rules_buf vs buf generate | Commit generated files. `make proto-gen` is the developer command; CI validates freshness, never generates. Bazel resolves Go deps from `go.mod` via gazelle's `go_deps` extension — `go.mod` stays the single source of truth for versions. |
| buf is the only protobuf generator | `# gazelle:proto disable_global` in the root `BUILD.bazel`. Left on, gazelle emits `proto_library` + `go_proto_library` rules claiming the same `importpath` as the committed buf output — two generators racing for one package. (It also resolved the imports to `//gcs/v1:*` labels that do not exist, so the rules did not build.) Bazel compiles the committed generated Go as ordinary source. |
| Bazel toolchain versions | `rules_go` 0.62.0, `gazelle` 0.52.2, bazel 7.4.1 via `.bazelversion`. **rules_go must keep pace with the Go SDK**: 0.52.0 passes `GOEXPERIMENT=coverageredesign`, removed in Go 1.25, and the stdlib build fails with `unknown GOEXPERIMENT coverageredesign`. Bump rules_go whenever go.mod's Go version moves. |
| Frontend under Bazel | `aspect_rules_js` + `aspect_rules_ts`. `npm_translate_lock` reads `frontend/pnpm-lock.yaml` — the lockfile is the source of truth for JS versions exactly as `go.mod` is for Go, and Bazel never restates a version. `frontend/BUILD.bazel` is hand-written; `# gazelle:exclude frontend` keeps gazelle (Go-only) out of it. `bazel test //...` now covers both languages. |
| ts_project typecheck gate | The real target is the rules_ts-generated `//frontend:typecheck_typecheck_test`, **not** `bazel build //frontend:typecheck` — typechecking runs in a separate action whose outputs live in the `typecheck` output group, so building the default outputs passes with type errors in the tree. Verified by canary. See ADR-0006 §4. |
| Toolchain version coupling | See **ADR-0006**. rules_go ↔ Go SDK ↔ gazelle, and aspect_rules_js ↔ Bazel version, are coupled: bumping one forces the others. Pinned at Bazel 8.7.0, rules_go 0.62.0, gazelle 0.52.2, aspect_rules_js 3.4.0, aspect_rules_ts 3.10.0, Go SDK 1.25.0. Bump them as one batch. |
| Ruleset telemetry | `aspect_rules_js` pulls in `aspect_tools_telemetry`, which collects ruleset usage data by default. Opted out in `.bazelrc` via `common --repo_env=ASPECT_TOOLS_TELEMETRY_OPTOUT=1` so it is a property of the checkout, not of whoever runs the build. |
| Go SDK hermeticity | `go_sdk.download(version = "1.25.0")`, **not** `go_sdk.host()`. ADR-0001 asks for no implicit dependency on ambient environment state; `host()` makes the build depend on whichever Go is installed. Keep the pinned version in step with `go.mod`. |
...
| HEARTBEAT's destination | **`HeartbeatState`, never a `TelemetryEvent` variant.** `TelemetryEvent`'s oneof has no heartbeat member and will not gain one. **Divergence from the predecessor, deliberate:** `flight-path-hud` was read-only MAVLink, where HEARTBEAT is one more message to render, so it rode the telemetry envelope with everything else. Here it carries *fleet identity* — discovery, loss, recovery, armed state, mode — and drives the vehicle fold, which is a different consumer with a different lifetime from a telemetry sample. Folding it into the telemetry envelope would mean every consumer of a position update also destructures a discovery event. Note the oneof additionally carries `pid_tuning`, `named_value_float` and `named_value_int`, which are *not* in the 18-family priority set — 12 + 3 = the 15 variants actually present. |
| Supported vehicle set | **Copter and Plane.** Everything vehicle-shaped is sized to this list and nothing else: SITL services, vendored parameter-metadata sets (ADR-0009 §4), and `scripts/check-tier-4.sh`'s supported-set assertion. Adding a type is three things — a `waf` target, a SITL service, a metadata set. |
...
| Manual control / joystick (`MANUAL_CONTROL` #69, `RC_CHANNELS_OVERRIDE` #70) | **Deferred, not overlooked** — it was absent from this table until 2026-08-18, which read as oversight. It is a *third write class*: unacknowledged and continuous, so incompatible with the command registry, the ACK correlation and the per-action audit Tier 8 is built on. It needs its own path — rate limiting, a deadman/heartbeat requirement, explicit take-control handoff, sampled audit, its own eligibility gate, and bandwidth arithmetic against the `RADIO_STATUS.txbuf` back-pressure signal the heartbeat-cardinality decision protects. Gate it on `MAV_PROTOCOL_CAPABILITY_COMPONENT_ACCEPTS_GCS_CONTROL` (524288), already in `types.proto` with no consumer, per ADR-0007. **Open tension:** the browser Gamepad API only reads while the tab is focused — the strongest Electron reopen-trigger on file; the alternative is native HID in the Go backend with the browser as display only. Design notes: `../research/cockpit-reference-reconciliation.md` §2 A. |
