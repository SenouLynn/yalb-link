...
### Navigation
Two stacked bars, each 28px minimum over a 1px inset — a bar of levers is a bar of borders, and at zero inset a lever's own edge fuses with the bar's rule. The app bar in Panel Graphite carries navigation left (the back action, then the vehicle picker once a vehicle is open) and the source badge right; it holds no title, because a row of chrome spent on a name that never changes is a row the fleet could have had. The view subbar in Panel Deep beneath it is lighter because it is the subordinate of the two, and it belongs to the **view** rather than to the shell: the vehicle pane raises one to hold the Views control against its right edge, and the fleet view raises one only when the replay transport needs somewhere to stand. Slot-level navigation is the tab strip: 10px `0.1em` uppercase Lume Dim tabs on a hairline, hovering to Lume on Panel Raised, selected by a 2px Lume bottom edge, with an optional right-aligned note (a count, a rate) pushed to the end of the strip. The Views popover is the one floating menu: Panel Graphite, 1px border, 2px radius, tier headings in the label step, options 28px tall on the target axis.
